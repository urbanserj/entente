                                       ...
                                       int tag_mode)
{
    ...
    ASN_DEBUG("Decoding %s as %s (frame %ld)", td->name,
              (type_variant == ASN_OSUBV_STR) ? "OCTET STRING"
                                              : "OS-SpecialCase",
              (long)size);
    ...
    case 0:
         ...
         * Check tags.
         */
        rval = ber_check_tags(opt_codec_ctx, td, ctx, buf_ptr, size, tag_mode,
                              -1, &ctx->left, &tlv_constr);
        if (rval.code != RC_OK)
            return rval;
    ...
    phase1:
        ...
        do {
            ...
            ssize_t Left = ((!sel || (size_t)sel->left >= size) ? (ssize_t)size
                                                                : sel->left);
            ...
            ASN_DEBUG("fetch tag(size=%ld,L=%ld), %sstack, left=%ld, "
                      "wn=%ld, tl=%ld",
            ...
            ll = ber_fetch_length(tlv_constr, (const char *)buf_ptr + tl,
                                  Left - tl, &tlv_len);
            ...
            if (tlv_tag != expected_tag) {
                ...
                ber_tlv_tag_snprint(td->tags[td->tags_count - 1], buf[1],
                                    sizeof(buf[1]));
                ASN_DEBUG("Tag does not match expectation: %s != %s", buf[0],
                          buf[1]);
                RETURN(RC_FAIL);
            }
            ...
            ASN_DEBUG("+EXPECT2 got=%ld left=%ld, wn=%d, clvl=%d",
                      (long)sel->got, (long)sel->left, sel->want_nulls,
                      sel->cont_level);
        } while (tlv_constr);
        if (sel == NULL) {
                                        ...
                                        void *sptr, asn_per_outp_t *po)
{
    ...
    ASN_DEBUG("Encoding %s into %d units of %d bits"
              " (%ld..%ld, effective %d)%s",
              td->name, sizeinunits, unit_bits, csiz->lower_bound,
