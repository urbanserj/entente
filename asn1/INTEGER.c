...
asn_TYPE_descriptor_t asn_DEF_INTEGER = {
    "INTEGER", "INTEGER", ASN__PRIMITIVE_TYPE_free, INTEGER_print,
    asn_generic_no_constraint, ber_decode_primitive, INTEGER_encode_der,
    INTEGER_decode_xer, INTEGER_encode_xer,
...
INTEGER_map_enum2value(asn_INTEGER_specifics_t *specs, const char *lstart,
                       const char *lstop);

/*
 * INTEGER specific human-readable output.
 */
static ssize_t INTEGER__dump(const asn_TYPE_descriptor_t *td,
                             const INTEGER_t *st, asn_app_consume_bytes_f *cb,
    ...
    if (ct && ct->flags != APC_UNCONSTRAINED) {
        ...
        if (ct->range_bits >= 0) {
            ...
            if (specs && specs->field_unsigned) {
                ...
                if (uper_get_constrained_whole_number(pd, &svalue,
                                                      ct->range_bits))
                    _ASN_DECODE_STARVED;
                ASN_DEBUG("Got value %ld + low %ld", svalue, ct->lower_bound);
                                   ...
                                   void *sptr, asn_per_outp_t *po)
{
    ...
    if (ct) {
        ...
        if (specs && specs->field_unsigned) {
            ...
            ASN_DEBUG("Value %lu (%02x/%d) lb %lu ub %lu %s", uval, st->buf[0],
            ...
            ASN_DEBUG("Value %ld (%02x/%d) lb %ld ub %ld %s", value, st->buf[0],
                      st->size, ct->lower_bound, ct->upper_bound,
                      inext ? "ext" : "fix");
        }
    ...
    if (ct && ct->range_bits >= 0) {
        /* #11.5.6 -> #11.3 */
        ASN_DEBUG("Encoding integer %ld (%lu) with range %d bits", value,
